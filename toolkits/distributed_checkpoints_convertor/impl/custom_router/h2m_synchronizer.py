...
def get_init_proj(expert_gate_projs, router_weights):
    """
    计算 Probe Vector 的初始化值。
    Args:
        expert_gate_projs: [Num_Experts, FFN_Hidden, Hidden] (Expert 的 Gate 权重) 
        router_weights: [Num_Experts, Hidden] (原始 Router 的权重)
    Returns:
        probe_vector: [FFN_Hidden]
    """
    # 转换为 float32 保证计算精度
    W_experts = expert_gate_projs.float() # [N, F, H]
    W_router = router_weights.float()     # [N, H]
    
    # 目标：找到 probe p (维度 F)，使得 p @ W_experts[i] ≈ W_router[i]
    # 对每个 Expert i: p^T @ W_experts[i] (F, H) -> (1, H) ≈ W_router[i]
    # 这相当于解线性方程组。
    
    # 这里提供一个简化的全局平均视角或者最小二乘解法
    # 简单方案：既然 Router 学的是 Input 的方向，Expert Gate 也是 Input 的方向
    # 我们可以计算 Expert Gate 和 Router Weight 的余弦相似度加权？
    
    # 如果完全按照 "Weight-Aware" 的定义：
    # Logits = Input @ (Probe @ Expert_Weights)^T
    # 原 Logits = Input @ Old_Router_Weights^T
    # 所以我们要让 (Probe @ Expert_Weights) ≈ Old_Router_Weights
    
    # 维度变换：
    # X: [N, F, H] -> 视为 [N*H, F] 的大矩阵 (每一列是 Expert 在某 hidden 维度的投影)
    # Y: [N, H]    -> 视为 [N*H] 的大向量
    # 求解 X @ p = Y
    
    N, F, H = W_experts.shape
    
    # Reshape for Least Squares
    # X: [N * H, F]
    X = W_experts.permute(0, 2, 1).reshape(-1, F)
    # Y: [N * H]
    Y = W_router.reshape(-1)
    
    # 最小二乘法求解: p = (X^T X)^(-1) X^T Y
    # 使用 torch.linalg.lstsq
    solution = torch.linalg.lstsq(X, Y).solution
    
    print(f"Initialized Probe Vector with shape: {solution.shape}, ")
    return solution.to(expert_gate_projs.dtype)
...
class HF2MGSynchronizer(BaseSynchronizer):
    ...
    def set_moe_layer_state(self, moe, hf_moe):
        ...
        print(f">>> Transforming Router Weights to Probe Vector for layer...")
        
        # A. 加载原始 HF Router 权重 [Num_Experts, Hidden]
        # 注意：load_tensor 返回的是 GPU 或 CPU tensor，取决于 self.device
        router_weight = self.load_tensor(hf_moe.gate.weight)
        # B. 提取所有 Expert 的 Gate Proj 权重
        # 这需要遍历 HF 的 expert 列表。
        # Qwen/Mixtral 的 HF 结构通常是: hf_moe.experts[i].gate_proj.weight
        expert_gate_projs = []
        num_experts = len(hf_moe.experts)

        # 这里的逻辑假设 HF 权重在单文件或通过 safe_open 能访问到
        for i in range(num_experts):
            # 针对 Qwen2MoE/Mixtral 的命名规则
            hf_expert = hf_moe.experts[i]
            # 加载 gate_proj.weight [FFN_Hidden, Hidden]
            w = self.load_tensor(hf_expert.gate_proj.weight)
            expert_gate_projs.append(w)

        # Stack 起来: [Num_Experts, FFN_Hidden, Hidden]
        all_experts_gate = torch.stack(expert_gate_projs, dim=0)

        # C. 计算初始化 Probe
        # 确保都在同一个设备上计算
        if router_weight.device != all_experts_gate.device:
            router_weight = router_weight.to(all_experts_gate.device)
            
        probe_vector = get_init_proj(all_experts_gate, router_weight)

        def test_probe_vector(probe_vector):
            # 检查是否全为零
            if torch.all(probe_vector == 0):
                return False, "The vector is all zeros."
            
            # 检查是否包含 NaN 值
            if torch.any(torch.isnan(probe_vector)):
                return False, "The vector contains NaN values."
            
            # 计算范数并检查是否在 0.01 到 10 之间
            norm_value = torch.norm(probe_vector)
            if not (0.01 <= norm_value <= 10):
                return False, f"Norm is out of the expected range: {norm_value.item()}"
            
            return True, f"Probe vector is valid. Is all 0: {torch.all(probe_vector == 0).item()}, Norm: {norm_value.item():.4f}"

        assert test_probe_vector(probe_vector)[0], test_probe_vector(probe_vector)[1]
        
        # D. 赋值给 Megatron 模型
        # 注意：Mcore 模型此时已经实例化在 GPU 上了 (self._mgmodel)
        # 我们直接原地修改数据
        target_device = moe.router.probe_vector.device
        target_dtype = moe.router.probe_vector.dtype

        moe.router.probe_vector.data.copy_(probe_vector.to(device=target_device, dtype=target_dtype))

        # 打印确认
        if self.tp_rank == 0 and self.pp_rank == 0 and self.ep_rank == 0:
            print(f"    Set probe_vector (norm: {moe.router.probe_vector.norm().item():.4f})")
